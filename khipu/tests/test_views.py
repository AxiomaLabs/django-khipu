...
class TestViews(TestCase):
    """
    Testear las vistas de khipu
    """

    def test_verificacion(self):
        """
        Testing de verifcacion view
        """
        # Test para cuando hay un error en el guardado del modelo.
        # El payment_id no existe
        with mock.patch('khipu.communication.Khipu.service') as mock_service:
            mock_service.return_value = {}
            response = self.client.post(reverse("verificacion"), data={})
            self.assertTrue(response.status_code == 400)

        # Testear cuando el monto NO es el mismo
        Payment.objects.create(payment_id='payment_id_testing', amount=122222)
        with mock.patch('khipu.communication.Khipu.service') as mock_service, \
                mock.patch('khipu.signals.payment_amount_error.send') as mock_signal_amount_error:
            mock_service.return_value = {
                'payment_id': 'payment_id_testing',
                'amount': '2000',
            }
            response = self.client.post(reverse("verificacion"), data={'notification_token': '2122'})
            self.assertTrue(mock_signal_amount_error.called)
            self.assertTrue(response.status_code == 200)

        # Testear cuando el receiver_id no es el correcto
        Payment.objects.create(payment_id='payment_id_testing_2', amount='2000')
        with mock.patch('khipu.communication.Khipu.service') as mock_service, \
                mock.patch('khipu.signals.payment_receiver_id_error.send') as mock_signal_receiver_error:
            mock_service.return_value = {
        ...
        with mock.patch('khipu.communication.Khipu.service') as mock_service, \
                mock.patch('khipu.signals.payment_successful.send') as mock_payment_successful:
            mock_service.return_value = {
                'payment_id': 'payment_id_testing_2',
                'amount': '2000',
